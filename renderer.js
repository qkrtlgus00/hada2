...
// UI 설정(테마/색/제목/사이드바접힘) — data.json에 저장(origin 무관 영구)
let prefs = { theme: 'light', colors: {}, viewColors: {}, appTitle: '하다', sidebarCollapsed: false, ytRepeat: 'off', manual: { ledger: false, deadlines: false, commissions: false } };
...
// Google 캘린더 이벤트 → 우리 Event 모델 (순수 함수, 문자열 파싱으로 타임존 안전)
function mapGoogleEvent(g) {
  if (!g || !g.start) return null;
  let date, start, end;
  if (g.start.dateTime) {
    const s = g.start.dateTime;                 // "2025-01-02T07:00:00+09:00"
    const e = (g.end && g.end.dateTime) || s;
    date = s.slice(0, 10);
    start = s.slice(11, 16);
    end = e.slice(0, 10) === date ? e.slice(11, 16) : '23:59';
  } else if (g.start.date) {                      // 종일 이벤트
    date = g.start.date;
    start = '00:00';
    end = '23:59';
  } else {
...
function scheduleSave() {
  ...
  saveTimer = setTimeout(async () => {
    saveTimer = null;
    try { await window.api.save({ events, todos, recurring, ledger, deadlines, notes, habits, commissions, playlist, banner, bannerCfg, stickers, prefs }); }
...
// ---- Google 캘린더 연동 ----
function hasGoogle() { return typeof window !== 'undefined' && window.api && window.api.google; }

async function openGoogleModal() {
  if (!hasGoogle()) { toast('이 버전에는 Google 연동이 없어요. 파일을 모두 업데이트하세요.'); return; }
  const st = await window.api.google.status();
  const s = $('#g-status');
  if (st.connected) s.textContent = '연결됨';
  else if (st.hasCreds) s.textContent = '자격증명 저장됨 · 연결 필요';
  else s.textContent = '아직 설정 안 됨';
  $('#g-import').disabled = !st.connected;
  $('#g-signout').disabled = !st.connected;
  $('#g-connect').textContent = st.connected ? '다시 연결' : '저장 & 연결';
  $('#google-modal').hidden = false;
}
function closeGoogleModal() { $('#google-modal').hidden = true; }

// 구글 클라이언트 ID 형식 검증 (순수) — xxxxx.apps.googleusercontent.com
function isValidGoogleClientId(id) {
  return /\.apps\.googleusercontent\.com$/.test(String(id || '').trim());
}
async function googleConnect() {
  const clientId = $('#g-client-id').value.trim();
  const clientSecret = $('#g-client-secret').value.trim();
  if (clientId || clientSecret) {
    // 새 자격증명 입력 → 형식 검증 후 저장 (invalid_client 실수 조기 차단)
    if (!clientId || !clientSecret) { toast('클라이언트 ID와 시크릿을 모두 입력하세요.'); return; }
    if (!isValidGoogleClientId(clientId)) {
      toast('클라이언트 ID 형식이 올바르지 않아요. …apps.googleusercontent.com 으로 끝나야 해요. (ID와 시크릿을 바꿔 넣지 않았는지 확인)');
      return;
    }
    const r = await window.api.google.saveConfig({ clientId, clientSecret });
    if (!r.ok) {
      toast(r.error === 'BAD_CLIENT_ID'
        ? '클라이언트 ID 형식이 올바르지 않아요 (…apps.googleusercontent.com 으로 끝나야 함).'
        : '자격증명을 모두 입력하세요.');
      return;
    }
  }
  toast('브라우저에서 Google 로그인 창을 확인하세요… 오류가 나면 클라이언트 ID를 다시 확인하세요.');
  const res = await window.api.google.signIn();
  if (!res.ok) {
    const err = String(res.error || '');
    let msg;
    if (res.error === 'NO_CREDENTIALS') msg = '먼저 클라이언트 ID/시크릿을 입력하세요.';
    else if (/invalid_client/i.test(err)) msg = '구글이 클라이언트 ID를 인식하지 못했어요(invalid_client). Cloud Console에서 ID가 정확한지, 클라이언트가 삭제되지 않았는지, 같은 프로젝트인지 확인하세요.';
    else if (err === 'TIMEOUT') msg = '로그인 시간이 초과됐어요. 브라우저에 "invalid_client" 등 오류가 떴다면 클라이언트 ID를 다시 확인하세요.';
    else msg = `연결 실패: ${res.error}`;
    toast(msg);
    return;
  }
  toast(`Google 연결됨${res.email ? ' · ' + res.email : ''}`);
  await openGoogleModal();
...
async function importFromGoogle() {
  ...
  const r = await window.api.google.importEvents({ timeMin, timeMax });
  if (!r.ok) {
    toast(r.error === 'NOT_CONNECTED' ? '먼저 Google에 연결하세요.' : `가져오기 실패: ${r.error}`);
    return;
  }
  let added = 0, updated = 0;
  for (const g of r.items) {
    const ev = mapGoogleEvent(g);
    if (!ev) continue;
    const idx = events.findIndex((e) => e.googleId && e.googleId === ev.googleId);
    if (idx >= 0) { events[idx] = { ...events[idx], ...ev }; updated++; }
    else { events.push(ev); added++; }
  }
  scheduleSave();
  render();
  toast(`Google 일정 ${added}개 추가, ${updated}개 갱신`);
}
...
function ytEmbedSrc(t) {
  ...
  const common = 'rel=0&playsinline=1' + origin;
  if (t.videoId) return `${base}${t.videoId}?${common}${t.listId ? `&list=${t.listId}` : ''}`;
  if (t.listId) return `${base}videoseries?${common}&list=${t.listId}`;
  return '';
}
...
async function exportData() {
  ...
  const payload = { events, todos, recurring, ledger, deadlines, notes, habits, commissions, playlist, banner, bannerCfg, stickers, prefs };
...
function bindUI() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    else if (!$('#google-modal').hidden) closeGoogleModal();
  ...
  // 사이드바 메뉴 내비게이션 (Schedule=캘린더, Google=연동모달, 나머지=준비중 패널)
  document.querySelectorAll('[data-nav]').forEach((li) => {
    li.addEventListener('click', () => {
      if (li.id === 'nav-google') { openGoogleModal(); return; }
  ...
  $('#yt-form').addEventListener('submit', (e) => {
    ...
    if (!p.videoId && !p.listId) { toast('유효한 유튜브 링크가 아니에요.'); return; }
  ...
  // Google 연동 모달 버튼
  $('#g-close').addEventListener('click', closeGoogleModal);
  $('#google-modal').addEventListener('click', (e) => { if (e.target === $('#google-modal')) closeGoogleModal(); });
  $('#g-connect').addEventListener('click', googleConnect);
  $('#g-import').addEventListener('click', importFromGoogle);
  $('#g-signout').addEventListener('click', async () => {
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    fmt12, weekRangeLabel, minutesToTop, categoryColor, migrate, mapGoogleEvent,
    ...
    mix, stripHtml, ytEmbedSrc, ytWatchUrl, nextTrackId, resolveNextId, reorderByIds, isValidGoogleClientId, VIEW_COLORS, VIEW_META, viewAccent,
